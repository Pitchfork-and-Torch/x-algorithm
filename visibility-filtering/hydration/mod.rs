...
use std::sync::Arc;
use tes_hydrator::{retain_candidates_with_usable_media_or_url_entities, TesHydrator};
...
impl HydrationPipeline {
    ...
    pub(crate) async fn hydrate(&self, request: HydrationRequest<'_>) -> HydrationOutput {
        ...
        let candidate_hydration = async {
            ...
            let candidates =
                retain_candidates_with_usable_media_or_url_entities(candidates, &tes_tweet_keyed);
